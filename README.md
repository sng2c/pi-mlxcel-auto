...
- **metadata source**: remote-only (Hugging Face). Bare names are resolved as `MLXCEL_DEFAULT_ORG/<name>` (default `mlx-community`). If that 404s, the fallback context window (`MLXCEL_AUTO_MAXOUT`) is used — no search. No local file reading — works identically for mlxcel, mlx-lm, or any OpenAI-compatible server.

...
[pi](https://pi.dev) 확장 프로그램으로, 실행 중인 **`mlxcel-server`**([lablup/mlxcel](https://github.com/lablup/mlxcel)의 Rust MLX 러너, OpenAI 호환)를 자동 탐지하여 모델을 pi에 등록합니다. **모델 메타데이터에서 실제 컨텍스트 윈도우를 읽어오므로 `ctx-size`를 수동으로 입력할 필요가 없습니다.**
...
2. 각 모델 id에 대해:
   ...
   - 채팅 템플릿 도구 호출 마커 또는 `tool_parser_type`에서 도구를 감지합니다(참고용).
